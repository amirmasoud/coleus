<template>
  <b-row>
    <b-col cols="12"
           sm="4"
           md="4">
      <!--<draggable v-model="myArray" class="book-contents" v-if="book">-->
        <!--<transition-group :options="{draggable:'.item', handle:'.handle'}">-->
          <!--<div v-for="element, index in book.pages" :key="element.id" class="item">-->
            <!--<img class="handle" :src="'/icons/grab.svg'" /> {{element.title}}-->
          <!--</div>-->
        <!--</transition-group>-->
      <!--</draggable>-->
    ...
    <b-col cols="12"
      ...
      <ApolloQuery v-if="book" :query="require('~/graphql/page.gql')" :variables="{ id: id || book.pages[0].id }">
        <template slot-scope="{ result: { data, loading } }">
          <div v-if="loading" class="loading">Loading...</div>
          <div v-else-if="data">
            <div class="row" v-html="data.page.content"></div>
          </div>
        </template>
      </ApolloQuery>
