...
class scrapyghasidenaserSpider(scrapy.Spider):
    name = "scrapyghasidenaser"
    allowed_domains = ["ganjoor.net"]
    if 281 == 1:
        start_urls = ["https://ganjoor.net/naserkhosro/divann/ghaside-naser/sh"]
    else:
        start_urls = ["https://ganjoor.net/naserkhosro/divann/ghaside-naser/sh" + "1"]
    order = 1
    ...
    def parse(self, response):
        ...
        for i, poem in enumerate(response.css("div.poem>article>div")):
            ...
            if index == 0:
                if 1 == 1:
                    ...
                    sh["title"] = "قصیده" + " شماره " + str(self.order) + " - " + ''.join(response.css("div.poem>article>h2>a::text").extract()).strip() + ': ' + ''.join(poem.css("div.m1>p::text").extract()).strip()
                elif 1 == 4:
                    sh["title"] = "قصیده" + " شماره " + str(self.order) + " - " + ''.join(response.css("div.poem>article>h2>a::text").extract()).strip() + ': ' + ''.join(poem.css("div.m2>p::text").extract()).strip()
                else:
        ...
        if self.order < (281 + 1):
            next_page = response.urljoin("https://ganjoor.net/naserkhosro/divann/ghaside-naser/sh" + str(self.order))
