...
class golestanSpider(scrapy.Spider):
    name = "golestan"
    allowed_domains = ["ganjoor.net"]
    if 1 == 1:
        start_urls = ["https://ganjoor.net/saadi/golestan/dibache"]
    else:
        start_urls = ["https://ganjoor.net/saadi/golestan/dibache" + "1"]
    order = 1
    ...
    def parse(self, response):
        ...
        for i, poem in enumerate(response.css("div.poem>article>div")):
            ...
            if index == 0:
                if False:
                    sh["title"] = "بخش" + " شماره " + str(self.order) + " - " + poem.css("div.m1>p::text").extract_first()
                else:
        ...
        if self.order < (1 + 1):
            next_page = response.urljoin("https://ganjoor.net/saadi/golestan/dibache" + str(self.order))
