...
class scrapyabyataaSpider(scrapy.Spider):
    name = "scrapyabyataa"
    allowed_domains = ["ganjoor.net"]
    if 62 == 1:
        start_urls = ["https://ganjoor.net/abusaeed/abyat-aa/sh"]
    else:
        start_urls = ["https://ganjoor.net/abusaeed/abyat-aa/sh" + "1"]
    order = 1
    ...
    def parse(self, response):
        ...
        for i, poem in enumerate(response.css("div.poem>article>div")):
            ...
            if index == 0:
                if 2 == 1:
                    sh["title"] = "تکه" + " شماره " + str(self.order) + " - " + ''.join(poem.css("div.m1>p::text").extract()).strip()
                elif 2 == 2:
                    sh["title"] = "تکه" + " شماره " + str(self.order) + " - " + ''.join(poem.css("div.m2>p::text").extract()).strip()
                else:
        ...
        if self.order < (62 + 1):
            next_page = response.urljoin("https://ganjoor.net/abusaeed/abyat-aa/sh" + str(self.order))
