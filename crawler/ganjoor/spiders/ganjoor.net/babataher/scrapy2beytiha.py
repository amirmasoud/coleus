...
class scrapy2beytihaSpider(scrapy.Spider):
    name = "scrapy2beytiha"
    allowed_domains = ["ganjoor.net"]
    if 366 == 1:
        start_urls = ["https://ganjoor.net/babataher/2beytiha/sh"]
    else:
        start_urls = ["https://ganjoor.net/babataher/2beytiha/sh" + "1"]
    order = 1
    ...
    def parse(self, response):
        ...
        for i, poem in enumerate(response.css("div.poem>article>div")):
            ...
            if index == 0:
                if 1 == 1:
                    sh["title"] = "دوبیتی" + " شماره " + str(self.order) + " - " + ''.join(poem.css("div.m1>p::text").extract()).strip()
                elif 1 == 2:
                    sh["title"] = "دوبیتی" + " شماره " + str(self.order) + " - " + ''.join(poem.css("div.m2>p::text").extract()).strip()
                else:
        ...
        if self.order < (366 + 1):
            next_page = response.urljoin("https://ganjoor.net/babataher/2beytiha/sh" + str(self.order))
