...
class scrapyghazamSpider(scrapy.Spider):
    name = "scrapyghazam"
    allowed_domains = ["ganjoor.net"]
    if 61 == 1:
        start_urls = ["https://ganjoor.net/amir/ghazam/sh"]
    else:
        start_urls = ["https://ganjoor.net/amir/ghazam/sh" + "1"]
    order = 1
    ...
    def parse(self, response):
        ...
        if self.order < (61 + 1):
            next_page = response.urljoin("https://ganjoor.net/amir/ghazam/sh" + str(self.order))
