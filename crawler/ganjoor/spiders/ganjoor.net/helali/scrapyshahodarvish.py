...
class scrapyshahodarvishSpider(scrapy.Spider):
    name = "scrapyshahodarvish"
    allowed_domains = ["ganjoor.net"]
    if 53 == 1:
        start_urls = ["https://ganjoor.net/helali/shahodarvish/sh"]
    else:
        start_urls = ["https://ganjoor.net/helali/shahodarvish/sh" + "1"]
    order = 1
    ...
    def parse(self, response):
        ...
        for i, poem in enumerate(response.css("div.poem>article>div")):
            ...
            if index == 0:
                if 0 == 1:
                    ...
                    sh["title"] = "مثنوی" + " شماره " + str(self.order) + " - " + ''.join(response.css("div.poem>article>h2>a::text").extract()).strip() + ': ' + ''.join(poem.css("div.m1>p::text").extract()).strip()
                elif 0 == 4:
                    sh["title"] = "مثنوی" + " شماره " + str(self.order) + " - " + ''.join(response.css("div.poem>article>h2>a::text").extract()).strip() + ': ' + ''.join(poem.css("div.m2>p::text").extract()).strip()
                else:
        ...
        if self.order < (53 + 1):
            next_page = response.urljoin("https://ganjoor.net/helali/shahodarvish/sh" + str(self.order))
