...
class scrapykmbab03Spider(scrapy.Spider):
    name = "scrapykmbab03"
    allowed_domains = ["ganjoor.net"]
    if 3 == 1:
        start_urls = ["https://ganjoor.net/hojviri/kashfol-mahjoob/kmbab03/sh"]
    else:
        start_urls = ["https://ganjoor.net/hojviri/kashfol-mahjoob/kmbab03/sh" + "1"]
    order = 1
    ...
    def parse(self, response):
        ...
        if self.order < (3 + 1):
            next_page = response.urljoin("https://ganjoor.net/hojviri/kashfol-mahjoob/kmbab03/sh" + str(self.order))
