...
class scrapytarkibatoSpider(scrapy.Spider):
    name = "scrapytarkibato"
    allowed_domains = ["ganjoor.net"]
    if 2 == 1:
        start_urls = ["https://ganjoor.net/ouhadi/divano/tarkibato/sh"]
    else:
        start_urls = ["https://ganjoor.net/ouhadi/divano/tarkibato/sh" + "1"]
    order = 1
    ...
    def parse(self, response):
        ...
        for i, poem in enumerate(response.css("div.poem>article>div")):
            ...
            if index == 0:
                if 0 == 1:
                    sh["title"] = "ترکیب بند" + " شماره " + str(self.order) + " - " + ''.join(poem.css("div.m1>p::text").extract()).strip()
                elif 0 == 2:
                    sh["title"] = "ترکیب بند" + " شماره " + str(self.order) + " - " + ''.join(poem.css("div.m2>p::text").extract()).strip()
                else:
        ...
        if self.order < (2 + 1):
            next_page = response.urljoin("https://ganjoor.net/ouhadi/divano/tarkibato/sh" + str(self.order))
