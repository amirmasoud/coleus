...
class scrapymorabbaSpider(scrapy.Spider):
    name = "scrapymorabba"
    allowed_domains = ["ganjoor.net"]
    if 1 == 1:
        start_urls = ["https://ganjoor.net/ouhadi/divano/morabba/"]
    else:
        start_urls = ["https://ganjoor.net/ouhadi/divano/morabba/" + "1"]
    order = 1
    ...
    def parse(self, response):
        ...
        for i, poem in enumerate(response.css("div.poem>article>div")):
            ...
            if index == 0:
                if 0 == 1:
                    sh["title"] = "مربع" + " شماره " + str(self.order) + " - " + ''.join(poem.css("div.m1>p::text").extract()).strip()
                elif 0 == 2:
                    sh["title"] = "مربع" + " شماره " + str(self.order) + " - " + ''.join(poem.css("div.m2>p::text").extract()).strip()
                else:
        ...
        if self.order < (1 + 1):
            next_page = response.urljoin("https://ganjoor.net/ouhadi/divano/morabba/" + str(self.order))
