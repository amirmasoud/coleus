...
class scrapyghatghazSpider(scrapy.Spider):
    name = "scrapyghatghaz"
    allowed_domains = ["ganjoor.net"]
    if 20 == 1:
        start_urls = ["https://ganjoor.net/razi/ghatghaz/sh"]
    else:
        start_urls = ["https://ganjoor.net/razi/ghatghaz/sh" + "1"]
    order = 1
    ...
    def parse(self, response):
        ...
        for i, poem in enumerate(response.css("div.poem>article>div")):
            ...
            if index == 0:
                if True:
                    sh["title"] = "غزل و قطعه نا تمام" + " شماره " + str(self.order) + " - " + ''.join(poem.css("div.m1>p::text").extract()).strip()
                else:
        ...
        if self.order < (20 + 1):
            next_page = response.urljoin("https://ganjoor.net/razi/ghatghaz/sh" + str(self.order))
