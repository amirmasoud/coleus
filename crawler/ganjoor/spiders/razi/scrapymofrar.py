...
class scrapymofrarSpider(scrapy.Spider):
    name = "scrapymofrar"
    allowed_domains = ["ganjoor.net"]
    if 9 == 1:
        start_urls = ["https://ganjoor.net/razi/mofrar/sh"]
    else:
        start_urls = ["https://ganjoor.net/razi/mofrar/sh" + "1"]
    order = 1
    ...
    def parse(self, response):
        ...
        for i, poem in enumerate(response.css("div.poem>article>div")):
            ...
            if index == 0:
                if True:
                    sh["title"] = "مفرد" + " شماره " + str(self.order) + " - " + ''.join(poem.css("div.m1>p::text").extract()).strip()
                else:
        ...
        if self.order < (9 + 1):
            next_page = response.urljoin("https://ganjoor.net/razi/mofrar/sh" + str(self.order))
