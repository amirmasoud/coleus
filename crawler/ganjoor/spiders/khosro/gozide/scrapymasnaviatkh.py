...
class scrapymasnaviatkhSpider(scrapy.Spider):
    name = "scrapymasnaviatkh"
    allowed_domains = ["ganjoor.net"]
    if 100 == 1:
        start_urls = ["https://ganjoor.net/khosro/gozide/masnaviatkh/sh"]
    else:
        start_urls = ["https://ganjoor.net/khosro/gozide/masnaviatkh/sh" + "1"]
    order = 1
    ...
    def parse(self, response):
        ...
        if self.order < (100 + 1):
            next_page = response.urljoin("https://ganjoor.net/khosro/gozide/masnaviatkh/sh" + str(self.order))
