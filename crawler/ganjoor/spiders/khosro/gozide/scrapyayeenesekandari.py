...
class scrapyayeenesekandariSpider(scrapy.Spider):
    name = "scrapyayeenesekandari"
    allowed_domains = ["ganjoor.net"]
    if 5 == 1:
        start_urls = ["https://ganjoor.net/khosro/gozide/ayeene-sekandari/sh"]
    else:
        start_urls = ["https://ganjoor.net/khosro/gozide/ayeene-sekandari/sh" + "1"]
    order = 1
    ...
    def parse(self, response):
        ...
        if self.order < (5 + 1):
            next_page = response.urljoin("https://ganjoor.net/khosro/gozide/ayeene-sekandari/sh" + str(self.order))
