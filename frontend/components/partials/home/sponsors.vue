<template>
  <coleus-container class="py-12 text-center">
    <section class="px-4">
      ...
      <div v-for="(group, groupKey) in sponsors" :key="groupKey" class="text-center pb-8">
        <h2 class="uppercase text pb-8">
          <span class="pb-2 border-b-2 border-coleus-lightgreen">{{ groupKey }}</span>
        </h2>
        <a
      ...
      <div class="text-center">
        <coleus-button :to="{ name: 'sponsor-nuxtjs' }" class="mr-4 py-3 px-6 text-base">
          <coleus-svg-go slot="icon" class="h-5 -mt-1 mr-1" />
          {{ buttonWording }}
        </coleus-button>
      </div>
    </section>
...
<script>
import coleusSvgGo from '@/components/svg/OpenCollective.vue'
