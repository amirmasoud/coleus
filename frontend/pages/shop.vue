    ...
    <div ref="shop" class="flex flex-wrap justify-between mb-8">
      <div class="lg:w-6/12 lg:text-left text-center p-4 sm:p-0">
        <h1 class="text-3xl xl:text-4xl text-coleus-gray font-medium leading-normal mb-6 lg:pt-4">
          The NUXT<span class="text-coleus-lightgreen">JS</span> Shop<br>
        </h1>
      ...
      <div v-for="(product, i) in products" :key="i" class="w-full lg:w-1/2 p-4">
        <div class="block sm:flex w-full bg-gray-100 hover:bg-gray-200 p-6 rounded">
          ...
          <div class="w-full px-6 py-2">
            ...
            <button class="inline-block bg-coleus-lightgreen text-white font-medium text-sm px-4 py-2 shadow uppercase rounded hover:bg-coleus-green hover:shadow-md sm:mr-4 p-3 mt-3 text-sm text-left">
              Coming soon
            </button>
            <!-- <button class="snipcart-add-item inline-block bg-coleus-lightgreen text-white font-medium text-sm px-4 py-2 shadow uppercase rounded hover:bg-coleus-green hover:shadow-md sm:mr-4 p-3 mt-3 text-sm text-left" :data-item-id="i + 1" :data-item-name="product.name" :data-item-description="product.description" :data-item-image="'/shop/' + product.img + '.png'" :data-item-price="product.price" :data-item-url="{ name: 'shop' }">
            </button> -->
          </div>
        </div>
      </div>
